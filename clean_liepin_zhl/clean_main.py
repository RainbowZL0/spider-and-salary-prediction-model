...
class DataCleaning:
    ...
    def drop_rows_with_null(self, column_name):
        self.dataframe.dropna(axis=0, how='any', subset=[column_name], inplace=True)
        self.dataframe.reset_index(inplace=True, drop=True)
    ...
    def string_format(self):
        ...
        """
        self.dataframe.loc[:, '公司'] = [str(x).strip("[").strip("]").strip("'") for x in self.dataframe.loc[:, '公司']]
    ...
    def clean_places(self):
        ...
        self.dataframe.loc[:, '工作地点'] = [str(x).replace('-', '·').strip("[]")
                                         for x in self.dataframe.loc[:, '工作地点']]

        # 修改城市列。列名为"工作地点"。
        self.dataframe.loc[:, '工作地点'] = [str(x).split('·')[0].strip("[]")
                                         for x in self.dataframe.loc[:, '工作地点']]
    ...
    def clean_column_payment(self):
        ...
        self.dataframe = self.dataframe.loc[~self.dataframe['薪水'].str.contains('面议')]
        ...
        try:
            for index in payment_column.index:
                ...
                value_split[0] = value_split[0] + '000'
                if len(value_split) == 3:
                    modification = int(value_split[2].strip('薪')) / 12
                    value_split[0] = int(value_split[0]) * modification
                    value_split[1] = int(value_split[1]) * modification
    ...
    def clean_education(self):
        self.dataframe.loc[:, '学历要求'] = [str(x).strip('统招').strip("及以上").strip('学历')
                                         for x in self.dataframe.loc[:, '学历要求']]
    ...
    def clean_column_experience(self):
        ...
        # self.dataframe.loc[:, '工作经验'] = [str(x).strip('统招').strip("及以上").strip('学历')
        #                                  for x in self.dataframe.loc[:, '工作经验']]

        self.dataframe = self.dataframe.loc[~self.dataframe['工作经验'].str.contains('经验不限')]
        experience_column = self.dataframe["工作经验"]

        min_value_list, max_value_list = list(), list()
        for index, value in experience_column.items():
            value = value.strip('经验')  # Remove "经验" in the first place.
            value = value.replace(" ", "")
            if "-" in value:
